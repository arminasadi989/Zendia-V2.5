...
export const getDetailedSupplementaryNews = async (newsText: string): Promise<SupplementaryNewsResult> => {
  const prompt = `
    ...
    2. جستجوی واقعی در گوگل برای این خبر اجباری و غیرقابل‌حذف است؛ پیش از نوشتن پاسخ، حتماً باید ابزار جستجوی وب را واقعاً صدا بزنید و حداقل یک منبع خبری واقعی و معتبر (مثل سایت خبرگزاری، روزنامه، یا رسانهٔ شناخته‌شده) برای همین خبر پیدا کنید. صرفاً اتکا به دانش قبلی خود بدون انجام جستجوی واقعی، قابل قبول نیست.
