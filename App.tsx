...
import { Activity, Mic2, Key, Eye, EyeOff, Check, Trash2, AlertTriangle, ExternalLink, Sparkles, Settings2, AudioLines } from 'lucide-react';
import { setCustomApiKey, getCustomApiKey } from './services/geminiService';
import {
  getTtsProvider, setTtsProvider as persistTtsProvider, TtsProvider,
  getElevenLabsApiKey, setElevenLabsApiKey,
  getElevenLabsVoiceId, setElevenLabsVoiceId,
...
const App: React.FC = () => {
  ...
  const [ttsProvider, setTtsProviderState] = useState<TtsProvider>(getTtsProvider());
  const [elevenLabsKeyValue, setElevenLabsKeyValue] = useState<string>(getElevenLabsApiKey() || '');
  const [isElevenLabsKeyVisible, setIsElevenLabsKeyVisible] = useState<boolean>(false);
  const [isElevenLabsKeySaved, setIsElevenLabsKeySaved] = useState<boolean>(false);
  const [elevenLabsVoiceIdValue, setElevenLabsVoiceIdValue] = useState<string>(getElevenLabsVoiceId());
  ...
  const handleApplyElevenLabsVoiceId = (voiceId: string) => {
    const trimmed = voiceId.trim();
    setElevenLabsVoiceId(trimmed || null);
    setElevenLabsVoiceIdValue(trimmed || getElevenLabsVoiceId());
  };
      ...
      {isQuotaModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl relative flex flex-col gap-4 overflow-hidden animate-in scale-in duration-200" dir="rtl">
            ...
            <div className="z-10 space-y-3 pt-3 border-t border-slate-800/60">
              <h4 className="text-[10px] text-cyan-400 font-extrabold flex items-center gap-1.5">
                <Settings2 className="w-3.5 h-3.5" />
                تنظیمات تبدیل صوت (موتور TTS)
              </h4>
              ...
              <div className="grid grid-cols-2 gap-2 bg-slate-950/60 p-1 rounded-xl border border-slate-800/80">
                <button
                  ...
                  className={`py-2 rounded-lg text-[11px] font-black transition-all flex items-center justify-center gap-1.5 focus:outline-none cursor-pointer ${
                    ttsProvider === 'elevenlabs' ? 'bg-cyan-500 text-slate-950' : 'text-slate-400 hover:text-slate-200'
                  }`}
                >
                ...
                <button
                  type="button"
                  onClick={() => handleSelectTtsProvider('gemini')}
                  className={`py-2 rounded-lg text-[11px] font-black transition-all flex items-center justify-center gap-1.5 focus:outline-none cursor-pointer ${
                    ttsProvider === 'gemini' ? 'bg-cyan-500 text-slate-950' : 'text-slate-400 hover:text-slate-200'
                  }`}
                >
              ...
              {ttsProvider === 'elevenlabs' && (
                <div className="space-y-2.5 bg-slate-950/40 p-3 rounded-2xl border border-slate-800/60">
                  <label className="text-[10px] text-cyan-400 font-extrabold flex items-center gap-1">
                    <Key className="w-3.5 h-3.5 text-cyan-400" />
                    کلید API الون‌لبز (ElevenLabs API Key)
                  </label>
                  <div className="relative">
                    <input
                      type={isElevenLabsKeyVisible ? "text" : "password"}
                    ...
                    <button
                      type="button"
                      onClick={() => setIsElevenLabsKeyVisible(!isElevenLabsKeyVisible)}
                      className="absolute inset-y-0 left-3 flex items-center text-slate-500 hover:text-cyan-400 transition-colors focus:outline-none cursor-pointer"
                    >
                      {isElevenLabsKeyVisible ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                    </button>
                  </div>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-slate-400 hover:text-cyan-300 flex items-center gap-1 transition-colors font-bold text-[9px] px-1.5"
                  >
                    دریافت/ساخت کلید از پنل ElevenLabs
                    <ExternalLink className="w-3 h-3" />
                  </a>

                  <label className="text-[10px] text-cyan-400 font-extrabold flex items-center gap-1 pt-1">
                    شناسه صدا (Voice ID) — اختیاری
                  </label>
                  <input
                    type="text"
                    value={elevenLabsVoiceIdValue}
                    onChange={(e) => setElevenLabsVoiceIdValue(e.target.value)}
                    onBlur={(e) => handleApplyElevenLabsVoiceId(e.target.value)}
                    placeholder="21m00Tcm4TlvDq8ikWAM"
                    dir="ltr"
                    className="w-full bg-slate-950 border border-slate-800/80 focus:border-cyan-500 rounded-xl px-3 py-2 text-xs font-mono text-slate-300 focus:outline-none transition-all"
                  />

                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <button
                      type="button"
                      onClick={() => handleApplyElevenLabsKey(elevenLabsKeyValue)}
                      disabled={!elevenLabsKeyValue.trim()}
                      className={`py-2 px-4 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 shadow-md border focus:outline-none cursor-pointer ${
                        !elevenLabsKeyValue.trim()
                          ? 'bg-slate-800/40 text-slate-600 border-slate-800/50 cursor-not-allowed'
                          : isElevenLabsKeySaved
                            ? 'bg-emerald-600 border-emerald-500 text-white'
                            : 'bg-cyan-500 border-cyan-400 hover:bg-cyan-400 text-slate-950 hover:text-white'
                      }`}
                    >
                      {isElevenLabsKeySaved ? (<><Check className="w-4 h-4" />ثبت شد!</>) : (<><Check className="w-4 h-4" />ذخیره کلید</>)}
                    </button>
                    <button
                      type="button"
                  ...
                  <p className="text-[9px] text-slate-500 text-center leading-relaxed">
                    این کلید روی همین مرورگر ذخیره می‌شود و بعد از ریفرش صفحه باقی می‌ماند.
                  </p>
                </div>
              )}
            </div>
